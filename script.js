...
camera.position.set(5, 5, 5); // TEMPORARY: Moved closer for testing
camera.lookAt(0, 0, 0);
...
const sphereFragmentShader = `
    void main() {
        gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0); // TEMPORARY: Solid red, no discard
    }
`;
...
function updateSpheres() {
    spheres.forEach(sphere => {
        const r = 1; // TEMPORARY: Fixed radius for testing
...
function nucleate() {
    ...
    for(let i = 0; i < n_attempt; i++) {
        ...
        if(!inside) {
            addSphere(p, t);
            console.log('Sphere added. Total spheres:', spheres.length); // TEMPORARY: Log for debugging
        }
    }
}
...
function animate() {
    ...
    console.log('Animating at t =', t); // TEMPORARY: Log for debugging
    t += dt;
    updateSpheres();
    nucleate();
    updateUniforms();
