...
renderer.setClearColor(0x000000); // Set background to black to avoid white screen
...
// Use a basic material for spheres to ensure visibility
...
function addSphere(center, t_n) {
    ...
    const mesh = new THREE.Mesh(geometry, sphereMaterial); // Use basic material
    mesh.position.copy(center);
    mesh.scale.set(1, 1, 1); // Fixed scale for testing
    scene.add(mesh);
    spheres.push({ mesh, center, t_n });
    console.log('Sphere added at position', center, 'at time', t_n, 'Total:', spheres.length);
}

function updateSpheres() {
    spheres.forEach((sphere) => {
        const r = c * (t - sphere.t_n); // Dynamic scaling based on time
...
function updateUniforms() {
    // Not needed with basic material; can be reintroduced with shaders later
}

function updateIntersections() {
    // Optional: Uncomment to enable intersection lines once spheres are visible
    /*
        ...
        for (let j = i + 1; j < spheres.length; j++) {
            ...
            if (d < r1 + r2 && d > Math.abs(r1 - r2)) {
                ...
                for (let k = 0; k < numPoints; k++) {
                    ...
                    points.push(
                        center.clone()
                            .add(tangent.clone().multiplyScalar(h * Math.cos(angle)))
                            .add(bitangent.clone().multiplyScalar(h * Math.sin(angle)))
                    );
                }
...
function animate() {
    ...
    t += dt;
    updateSpheres(); // Re-enabled to show growing spheres
    nucleate();
    // updateUniforms(); // Not needed with basic material
    // updateIntersections(); // Uncomment to enable intersection lines
